...
**Status:** Completo — engine, correção de bug crítico de SVG, helper visual e ícones reais do Mana implementados e validados. Ver seções 11–13 para o registro de cada rodada.
...
4. ~~Implementar o helper visual no editor de template~~ — feito, ver seção 12 (e também na tela de Dados, não só no editor).
5. ~~Escrever a página de wiki~~ — feito, `docs/09-simbolos-mana.md`.
6. ~~Investigar e corrigir o bug de unidades encontrado em `svg_builder.py`~~ — feito, ver seção 11.5.
