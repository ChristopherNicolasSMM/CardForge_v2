...
**Status:** Implementado e validado. Validação própria feita localmente com o equivalente Linux (ver seção 7); build Windows real gerado e confirmado funcionando pelo usuário (ver seção 8.1).
...
### 8.1 Build Windows — confirmado funcionando

O build Windows real foi gerado e confirmado rodando pelo usuário. A validação própria feita neste documento (seção 7) usou um equivalente Linux por não haver acesso a uma máquina Windows durante o desenvolvimento — a confirmação direta no SO alvo fecha essa lacuna.

### 8.2 Demais pontos em aberto

- **Ícone do executável:** o build Linux ignorou o `.ico` (esperado — `.ico`/`.icns` só são aplicados em Windows/Mac pelo PyInstaller). Como o build Windows já está confirmado funcionando (seção 8.1), vale só um check visual rápido de que o ícone aparece certo no `.exe`/na barra de tarefas.
