...
> Os ícones de símbolo de mana (`{W}`, `{T}` etc.) são um recurso global separado, sempre disponível em qualquer coleção/template — não precisam de upload. Veja [Símbolos de mana](09-simbolos-mana).
