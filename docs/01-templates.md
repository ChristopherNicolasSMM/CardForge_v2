...
| Texto fixo | Usado em camadas de **texto** quando o campo está vazio — um texto que não muda entre cards. Aceita notação de símbolo inline (ex: `{W}`, `{T}`) — veja [Símbolos de mana](09-simbolos-mana) — e tem um botão **🔮 Símbolo** ao lado do campo pra inserir sem decorar a notação. |
...
O tipo **custo/ícone** é tecnicamente igual a **texto** — existe como opção separada só pra deixar claro, ao olhar a lista de camadas, qual delas é o custo de mana. Qualquer camada de texto (dos dois tipos) aceita notação `{W}`, `{T}` etc. — veja [Símbolos de mana](09-simbolos-mana) — não é exclusivo do tipo custo/ícone.
