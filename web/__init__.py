...
def create_app() -> Flask:
    ...
    from core.render.mana_symbols import set_collection_icons_dir, reset_collection_icons_dir

    # Migração automática da coleção "Geral" DESATIVADA a pedido — o app não
    # deve mais criar (nem recriar) uma coleção "Geral" sozinho a partir de
    # templates/assets legados. A função continua disponível em
    # web/services/collections.py:migrate_legacy_if_needed() caso alguém
    # queira rodá-la manualmente/reativar no futuro.
    #
    ...
    def _bind_active_collection():
        ...
        if g.collection:
            ...
            g._cf_tokens = (
                ...
                set_collection_icons_dir(collections_service.icons_dir(g.collection)),
            )
    ...
    def _unbind_active_collection(exc):
        ...
        if tokens:
            t_templates, t_fonts, t_icons = tokens
            reset_templates_root(t_templates)
            reset_custom_fonts_dir(t_fonts)
            reset_collection_icons_dir(t_icons)
