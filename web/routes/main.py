...
from flask import Blueprint, render_template, g, jsonify, send_from_directory, abort
...
# Catálogo embutido (estilo MTG) sempre disponível globalmente, mais as
# notações customizadas da coleção ativa (collections/<coleção>/assets/
# icons_png/manifest.json), resolvidas via ContextVar setado por requisição
# em web/__init__.py — mesmo padrão de core/template/loader.py (templates
# root) e core/render/font_paths.py (fontes custom). Usado pelo helper
# visual do editor e da tela de Dados (ver web/static/js/symbol-picker.js).
# Ver docs/09-simbolos-mana.md.
...
def symbols_icon(filename):
    resolved = mana_symbols.resolve_icon_file(filename)
    if resolved is None:
        abort(404)
