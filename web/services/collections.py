...
uma pasta própria e autocontida em collections/<slug>/:
  ...
  collections/<slug>/
    ...
    assets/icons_png/       ← ícones/símbolos customizados dessa coleção
                               (opcional — ver core/render/mana_symbols.py)
...
def icons_dir(slug: str) -> Path:
    """Pasta de ícones/símbolos customizados dessa coleção — opcional, usada
    por core/render/mana_symbols.py pra estender ou sobrescrever o catálogo
    embutido (estilo MTG) sem tocar em assets/icons_png/ global. Ver
    manifest.json em core/render/mana_symbols.py para o formato."""
    d = collection_dir(slug) / "assets" / "icons_png"
    d.mkdir(parents=True, exist_ok=True)
    return d
...
def proxy_quantities_path(slug: str) -> Path:
    return collection_dir(slug) / "proxy_quantities.json"
...
def resolve_identifier_field(slug: str, columns: list[str]) -> str | None:
    """Campo usado pra identificar visualmente cada linha do dataset (ex: na
    lista de quantidade do Proxy). Prioridade:
      1. Campo explicitamente escolhido na tela de Dados (`identifier_field`
         em collection.json), se ainda existir entre as colunas atuais.
      2. `name`, se existir.
      3. A primeira coluna do dataset.
      4. None (quem chama cai pro número da linha)."""
    meta = read_meta(slug)
    explicit = (meta.identifier_field or "").strip()
...
def create_collection(name: str, description: str = "", game: str = "",
    ...
    (d / "assets" / "icons_png").mkdir(parents=True, exist_ok=True)
...
def delete_collection(slug: str) -> None:
    d = collection_dir(slug)
    if d.exists():
        from core.fsutil import rmtree_retry
        rmtree_retry(d)
...
def duplicate_collection(src_slug: str, new_name: str,
    ...
    if include_assets:
        for sub in ("library", "fonts_custom", "icons_png"):
